...
* Перед началом прохождения студент может выбрать: отвечать **анонимно** или **с указанием имени**.
...
* **Уведомления кураторов**: После отправки фидбека все кураторы курса автоматически получают уведомление о новом отзыве с указанием темы, содержания и информации об анонимности.
...
- Для получения уведомлений о новых отзывах кураторы должны запустить бота и выполнить любую команду куратора (например, /set_group).
- При первом использовании бота система автоматически сохраняет Telegram ID куратора для отправки уведомлений.
- Уведомления отправляются всем кураторам курса, которые зарегистрированы в системе и имеют сохранённый Telegram ID.

### Система логирования
- Бот автоматически создает файлы логов в папке `logs/` с названием формата `bot_YYYY-MM-DD.log`
- Каждый день в полночь автоматически создается новый файл лога
- Логи хранятся в течение 30 дней, после чего старые файлы автоматически удаляются
- По умолчанию все события записываются только в файлы (не дублируются в консоль)
- Формат логов: `[YYYY-MM-DD HH:MM:SS] LEVEL:module_name:message`
- Для разработки можно включить вывод в консоль через параметр `console_output=True`
...
- list_curators
  - Показывает список всех кураторов в системе с подробной информацией:
    - Имя пользователя и ID куратора
    - Telegram ID (если куратор зарегистрирован в боте)
    - Список курсов, к которым привязан куратор
    - Статус возможности получения уведомлений
  - Отображает сводку: сколько кураторов могут/не могут получать уведомления
  - Показывает подсказку для кураторов без Telegram ID о необходимости запустить бота
- list_links
  - Показывает ссылки на Google Sheets таблицы:
    - Таблица с отзывами студентов (feedback)
    - Таблица с результатами прохождения опросов (survey responses)
  - Удобно для быстрого доступа к данным для анализа
...
Бот имеет возможность сохранять отзывы студентов и ответы на опросы в Google Sheets, что позволяет удобно анализировать и обрабатывать обратную связь. Для настройки интеграции необходимо:
...
2. **Настройте Google Sheets**:
   - Создайте две таблицы Google Sheets:
     - Одну для отзывов (feedback)
     - Другую для ответов на опросы (survey responses)
   - Скопируйте ID таблиц из URL (например, из https://docs.google.com/spreadsheets/d/YOUR_SHEET_ID/edit)
   - Предоставьте доступ к таблицам для email-адреса сервисного аккаунта (он указан в JSON-файле) с правами редактирования

3. **Настройте переменные окружения**:
   - Отредактируйте файл `.env` и добавьте следующие переменные:
     ```
     # Для отзывов
     GSHEET_ID=ваш_id_таблицы_отзывов
     GSHEET_URL=полный_url_таблицы_отзывов
     GSHEET_TAB_NAME=название_вкладки (по умолчанию "feedback")
     
     # Для ответов на опросы
     SURVEYS_GSHEET_URL=полный_url_таблицы_опросов
     SURVEYS_GSHEET_TAB_NAME=название_вкладки (по умолчанию "survey_responses")
...
После этих настроек:
- Каждый отзыв, отправленный через команду `/feedback`, будет автоматически сохраняться в первую таблицу с указанием времени, имени пользователя, названия курса, темы и текста отзыва.
- Каждый ответ на вопрос опроса будет сохраняться во вторую таблицу с подробной информацией: время, имя студента, курс, группа, название опроса, текст вопроса, тип вопроса и ответ.
